% !TEX TS-program = pdflatex
% !BIB TS-program = biber
\documentclass[11pt,letterpaper]{article}
%\usepackage[top=0.75in,bottom=0.75in,left=0.5in,right=0.5in]{geometry}
\usepackage{multicol}

\usepackage[backend=biber,bibstyle=academic,defernumbers=true,sorting=nty]{biblatex}
\setboolean{money}{true}

\addbibresource{./biblatex-academic.bib}

\renewcommand{\bibfont}{\footnotesize}

\newcommand{\pkg}[1]{\textsf{#1}}
\newcommand{\entry}[1]{\textsf{#1}}
\newcommand{\field}[1]{\textsf{#1}}
\newcommand{\env}[1]{\textsf{#1}}
\newcommand{\sort}[1]{\textsf{#1}}
\newcommand{\command}[1]{\(\backslash\)#1}

\author{Jeffrey Wildman%
  \thanks{E-mail:\href{mailto:jeffrey.wildman@gmail.com}%
  {\texttt{jeffrey.wildman@gmail.com}}}
}
\title{The \pkg{academic} bibliography style for \pkg{biblatex}%
  \footnote{This file describes v0.0.1 last revised 2015/01/21.}%
}
\date{Released 2015/01/15}


\begin{document}

\maketitle

The \pkg{biblatex-academic} package provides a \pkg{biblatex} style for lists of things an academic might like to list in a CV or lab whitepaper (students, alumni, collaborators, proposals, projects, courses, etc.).
This implementation follows standard \pkg{biblatex} conventions, and can be used simply by loading \pkg{biblatex} with the appropriate options:
\begin{verbatim}
    \usepackage[backend=biber,bibstyle=academic]{biblatex}
\end{verbatim}
The \pkg{biblatex-academic} style requires \pkg{biblatex} 2.7a or later, requires \pkg{Biber} as the database back-end, and the following packages/styles: \pkg{biblatex-ieee}, \pkg{graphicx}, \pkg{siunitx}, \pkg{calc}, \pkg{hyperref}, and \pkg{wrapfig}.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Entry Types
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Entry Types}\label{sec:entry-types}

\pkg{biblatex-academic} defines several new entry types:
\entry{alum},
\entry{collaborator},
\entry{committee},
\entry{course},
\entry{presentation},
\entry{project},
\entry{proposal},
\entry{seniordesign},
\entry{service},
\entry{student}.

\begin{itemize}
  \item[\entry{alum}] An alum of a lab, university, etc.

    Required fields:
    \field{name},
    \field{image},
    \field{degree},
    \field{date},
    \field{thesis}

    Optional fields:
    \field{coadvisor},
    \field{coadvisor\_url},
    \field{employer},
    \field{employer\_url}

  \item[\entry{collaborator}] A research collaborator.

    Required fields:
    \field{name},
    \field{image},
    \field{position},
    \field{employer},
    \field{type}

    Optional fields:
    \field{employer\_url},
    \field{email},
    \field{website},
    \field{website\_alt}

  \item[\entry{committee}] A committee served on.

    Required fields:
    \field{title},
    \field{date},
    \field{student},
    \field{advisor},
    \field{chair},
    \field{member},
    \field{type}

    Optional fields:
    \field{coadvisor}

  \item[\entry{course}] A course course taught at a university.

    Required fields:
    \field{number},
    \field{title},
    \field{term},
    \field{academic\_year},
    \field{level},
    \field{enrollment},
    \field{instr\_mean},
    \field{instr\_std},
    \field{student\_mean},
    \field{student\_std}

    Optional fields:

  \item[\entry{presentation}] A presentation.
    Required fields:
    \field{presenter},
    \field{title},
    \field{eventtype},
    \field{event},
    \field{location},
    \field{date}

    Optional fields:

  \item[\entry{project}] A research project.

    Required fields:
    \field{title},
    \field{program},
    \field{investigators\_p},
    \field{duration},
    \field{amount},
    \field{status}

    Optional fields:
    \field{award\_num},
    \field{award\_url},
    \field{investigators\_co},
    \field{note}

  \item[\entry{proposal}] A research proposal.

    Required fields:
    \field{title},
    \field{program},
    \field{investigators\_p},
    \field{date},
    \field{amount},
    \field{status}

    Optional fields:
    \field{investigators\_co},
    \field{note}

  \item[\entry{seniordesign}] A senior design project.

    Required fields:
    \field{year},
    \field{title},
    \field{team},
    \field{advisor},
    \field{student}

    Optional fields:
    \field{note}

  \item[\entry{service}] A self-contained acti of service to a professional community.

    Required fields:
    \field{type},
    \field{conf\_name},
    \field{location},
    \field{duration}

    Optional fields:
    \field{conf\_url},
    \field{note}

  \item[\entry{student}] A student under advisement.

    Required fields:
    \field{name},
    \field{image},
    \field{degree},

    Optional fields:
    \field{email},
    \field{website},
    \field{website\_alt},
    \field{coadvisor},
    \field{coadvisor\_url},
    \field{bio}
\end{itemize}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Fields
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Fields}\label{sec:fields}

\pkg{biblatex-academic} maps several new fields to existing fields defined by \pkg{biblatex}.
The new fields and their types are listed below:

\begin{itemize}
    \item[\field{academic\_year}] field (date)
    \item[\field{advisor}] list (name)
    \item[\field{amount}] field (literal)
    \item[\field{award\_num}] field (literal)
    \item[\field{award\_url}] field (uri)
    \item[\field{bio}] field (literal)
    \item[\field{chair}] list (name)
    \item[\field{coadvisor\_url}] field (uri)
    \item[\field{coadvisor}] (list) name
    \item[\field{conf\_name}] field (literal)
    \item[\field{conf\_url}] field (uri)
    \item[\field{degree}] field (literal)
    \item[\field{duration}] field (date)
    \item[\field{email}] field (literal)
    \item[\field{employer\_url}] field (uri)
    \item[\field{employer}] field (literal)
    \item[\field{enrollment}] field (literal)
    \item[\field{eventtype}] field (literal)
    \item[\field{event}] field (literal)
    \item[\field{image}] field (verbatim)
    \item[\field{instr\_mean}] field (literal)
    \item[\field{instr\_std}] field (literal)
    \item[\field{investigators\_co}] list (name)
    \item[\field{investigators\_p}] list (name)
    \item[\field{level}] field (literal)
    \item[\field{member}] list (name)
    \item[\field{name}] list (name)
    \item[\field{position}] field (literal)
    \item[\field{presenter}] list (name)
    \item[\field{program}] field (literal)
    \item[\field{status}] field (literal)
    \item[\field{student\_mean}] field (literal)
    \item[\field{student\_std}] field (literal)
    \item[\field{student}] (list) name
    \item[\field{team}] field (literal)
    \item[\field{term}] field (literal)
    \item[\field{thesis}] field (literal)
    \item[\field{type}] field (literal)
    \item[\field{website\_alt}] field (literal)
    \item[\field{website}] field (uri)
\end{itemize}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Environments
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Environments}\label{sec:environments}

\pkg{biblatex-academic} defines new environments that must be used with certain entry types:

\begin{itemize}
  \item[\env{people}] An environment for use with \entry{alum}, \entry{collaborator}, and \entry{student} entries.
  \item[\env{course}] An environment for use with \entry{course} entries.
\end{itemize}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Sorting Schemes
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Sorting Schemes}\label{sec:sorting}

\pkg{biblatex-academic} defines several sorting styles:

\begin{itemize}
  \item[\sort{course}] A sorting style for use with \entry{course} entries.
  \item[\sort{eventdate}] A sorting style for use with \entry{project}, \entry{proposal}, and \entry{service} entries.
  \item[\sort{ymdnt}] A sorting style for use with \entry{presentation}, \entry{seniordesign}, and \entry{committee} entries.
\end{itemize}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% People
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{People}\label{sec:people}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Students
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Students}\label{sec:people-current-phd}

\begin{refsection}
  \setlength{\bibitemsep}{\baselineskip}
  \nocite{*}
  \begin{multicols}{2}
    \raggedcolumns
    \printbibliography[heading=none,type=student,resetnumbers=true,omitnumbers=true,env=people]
  \end{multicols}
\end{refsection}


\clearpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Alumni
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Alumni}\label{sec:people-alumni}

\begin{refsection}
  \setlength{\bibitemsep}{\baselineskip}
  \nocite{*}
  \begin{multicols}{2}
    \raggedcolumns
    \printbibliography[heading=none,type=alum,resetnumbers=true,omitnumbers=true,env=people]
  \end{multicols}
\end{refsection}


\clearpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Collaborators
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Collaborators}\label{sec:collaborators}

% Construct collaborator filters
\defbibfilter{IntCollab}{%
  type=collaborator and subtype=internal
}
\defbibfilter{ExtCollab}{%
  type=collaborator and subtype=external
}

\begin{refsection}
  \setlength{\bibitemsep}{\baselineskip}
  \nocite{*}

  \subsubsection{Internal}\label{sec:people-collaborators-internal}

  \begin{multicols}{2}
    \raggedcolumns
    \printbibliography[heading=none,resetnumbers=true,omitnumbers=true,env=people,filter=IntCollab]
  \end{multicols}

  \subsubsection{External}\label{sec:people-collaborators-external}

  \begin{multicols}{2}
    \raggedcolumns
    \printbibliography[heading=none,resetnumbers=true,omitnumbers=true,env=people,filter=ExtCollab]
  \end{multicols}
\end{refsection}


\clearpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Projects
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{refsection}
  \setlength{\bibitemsep}{\baselineskip}
  \nocite{*}
  \begin{refcontext}[sorting=eventdate]
    \printbibliography[heading=bibnumbered,title=Projects,type=project,resetnumbers=true]
  \end{refcontext}
  \label{sec:projects}
\end{refsection}


\clearpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Proposals
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Construct proposal filter for all proposals not declined
\defbibfilter{NotDeclProposal}{%
  type=proposal and not subtype=declined
}

% Print proposal bibliography
\begin{refsection}
  \setlength{\bibitemsep}{\baselineskip}
  \nocite{*}
  \begin{refcontext}[sorting=eventdate]
    \printbibliography[heading=bibnumbered,title=Proposals,resetnumbers=true,filter=NotDeclProposal]
  \end{refcontext}
  \label{sec:proposals}
\end{refsection}


\clearpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Presentations
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{refsection}
  \setlength{\bibitemsep}{0in}
  \nocite{*}
  \begin{refcontext}[sorting=ymdnt]
    \printbibliography[heading=bibnumbered,title=Presentations,type=presentation,resetnumbers=true]
  \end{refcontext}
  \label{sec:presentations}
\end{refsection}


\clearpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Senior Design Teams
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{refsection}
  \setlength{\bibitemsep}{\baselineskip}
  \nocite{*}
  \begin{refcontext}[sorting=ymdnt]
    \printbibliography[heading=bibnumbered,title=Senior Design Teams,type=seniordesign,resetnumbers=true]
  \end{refcontext}
  \label{sec:seniordesign}
\end{refsection}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Student Committee
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\clearpage
\section{Student Committees}\label{sec:committee}

\begin{refsection}
  \setlength{\bibitemsep}{\baselineskip}
  \nocite{*}
  \begin{refcontext}[sorting=ymdnt]
    \printbibliography[heading=subbibnumbered,title=Candidacy Examination,subtype=phd-candidacy,resetnumbers=true]
    \printbibliography[heading=subbibnumbered,title=Ph.D. Thesis Proposal,subtype=phd-proposal,resetnumbers=true]
    \printbibliography[heading=subbibnumbered,title=Ph.D. Thesis Defense,subtype=phd-defense,resetnumbers=true]
    \printbibliography[heading=subbibnumbered,title=M.S. Thesis Defense,subtype=ms-defense,resetnumbers=true]
  \end{refcontext}
\end{refsection}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Service
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\clearpage
\section{Service}\label{sec:service}

\defbibfilter{service-oc}{%
  type=service and subtype=conference-organizing-committee%
}
\defbibfilter{service-tpc}{%
  type=service and subtype=conference-technical-program-committee%
}
\defbibfilter{service-other}{%
  type=service and not subtype=conference-organizing-committee and not subtype=conference-technical-program-committee%
}

\begin{refsection}
  \setlength{\bibitemsep}{0in}
  \nocite{*}
  \begin{refcontext}[sorting=eventdate]
    \printbibliography[heading=subbibnumbered,title=Conference Organizing Committees,filter=service-oc,resetnumbers=true]
    \printbibliography[heading=subbibnumbered,title=Conference Technical Program Committees,filter=service-tpc,resetnumbers=true]
    \printbibliography[heading=subbibnumbered,title=Other,notsubtype=conference-technical-program-committee,filter=service-other,resetnumbers=true]
  \end{refcontext}
\end{refsection}


\clearpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Teaching
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{refsection}
  \setlength{\bibitemsep}{0in}
  \nocite{*}
  \begin{refcontext}[sorting=course]
    \printbibliography[heading=bibnumbered,title=Teaching,type=course,env=course,prenote=course,resetnumbers=true]
  \end{refcontext}
  \label{sec:teaching}
\end{refsection}


\end{document}
